...
class KNN(BinaryClassifier):
    ...
    def predict(self, X):
        ...
        if self.trY.size == 0:
            ...
            for trX in self.trX:
                euclidean_distance, squaredDistanceSum = 0, 0
                i = 0
                for x in X:
                    squaredDistanceSum = squaredDistanceSum + math.pow((trX[i] - x), 2)
                    i=i+1
                euclidean_distance = math.sqrt(squaredDistanceSum)
                euclidean_distance_list.append(euclidean_distance)
            sorted_euclidean_distance = np.argsort(euclidean_distance_list)
            for neighbour in range(0,K):
                if len(self.trY) > neighbour:
                    if self.trY[sorted_euclidean_distance[neighbour]] > 0:
