    ...
    def trainDT(self, X, Y, maxDepth, used):
        ...
        if maxDepth <= 0 or len(util.uniq(Y)) <= 1:
            ...
            for d in range(D):
                ...
                leftYprime = util.mode(leftY)
                rightYprime = util.mode(rightY)
                count_left_errors = 0
                count_right_errors = 0
                count_left_errors = [count_left_errors + 1 for label in leftY if leftYprime != label]
                count_right_errors = [count_right_errors + 1 for item in rightY if rightYprime != item]
                error =  sum(count_right_errors) + sum(count_left_errors)    ### TODO: YOUR CODE HERE
