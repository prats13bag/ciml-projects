...
class LogisticLoss(LossFunction):
    ...
    def lossGradient(self, X, Y, Yhat):
        ...
        ### TODO: YOUR CODE HERE
        exponent_factor=exp(-Y * Yhat)
        return -sum((Y*exponent_factor)/(1+exponent_factor)*X.T, axis = 1)
...
class HingeLoss(LossFunction):
    ...
    def loss(self, Y, Yhat):
        ...
        ### TODO: YOUR CODE HERE
        #util.raiseNotDefined()
        hinge_loss = [1-(Y[i]*Yhat[i]) if 1-(Y[i]*Yhat[i])>=0 else 0 for i in range (0,len(Y))]
        return sum(hinge_loss)
    ...
    def lossGradient(self, X, Y, Yhat):
        ...
        ### TODO: YOUR CODE HERE
        #util.raiseNotDefined()
        hinge_gradient = [-Y[i] if Y[i]*Yhat[i]<=1 else 0 for i in range (0,len(Y))]
        return sum(hinge_gradient * X.T, axis = 1)
...
class LinearClassifier(BinaryClassifier):
    ...
    def train(self, X, Y):
        ...
        module gd.
        """
        a,num_features = X.shape
        self.weights   = zeros((num_features,), dtype=int)
        ...
        def func(w):
            ...
            Yhat = dot(w, X.T)    ### TODO: YOUR CODE HERE
